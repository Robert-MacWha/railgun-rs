...
impl TxBuilder {
    ...
    /// Builds the operations. The in_notes must all be on the same tree and held
    /// by the same address.
    ///
    ...
    pub fn build(self, in_notes: Vec<Note>) -> Result<Vec<Operation>, GroupError> {
        ...
        let group = Operation {
            tree_index: in_notes[0].tree_number,
            in_notes,
            out_notes,
            unshield_note,
        };
